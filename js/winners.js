...
function renderWinners(winners){
  ...
  let body;
  if(!winners){
    body = `<div class="score-empty">🔧 아직 추첨 기능이 연결되지 않았어요.<br>매주 금요일, 월~금 5일을 모두 완료한 학생 중<br>행운의 7명을 뽑아 여기에 보여줄 예정이에요!</div>`;
  } else if(!winners.date || !winners.list || winners.list.length === 0){
    body = `<div class="score-empty">🍀 아직 추첨 소식이 없어요.<br>매주 금요일 아침, 그 주 월~금을 모두 완료한 학생 중<br>행운의 7명을 추첨해요. 이번 주도 도전해 보세요!</div>`;
  } else {
    ...
    body = `
      <div class="winners-sub">${formatWinnerDate(winners.date)} 추첨 · 월~금 5일 모두 완료한 학생 중 무작위 선정</div>
